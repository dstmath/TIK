...
class Magisk_patch:
    ...
    def __init__(
        ...
        self.magiskboot = os.path.join(
            local, "bin", platform.system(), platform.machine(), "magiskboot"
        )
    ...
    def check(self):
        ...
        self.STATUS = (
            self.exec("cpio", "ramdisk.cpio", "test")
        ...
        if (self.STATUS & 3) == 0:
            ...
            if os.path.exists(os.path.join(local, "ramdisk.cpio")):
                shutil.copyfile(
                    os.path.join(local, "ramdisk.cpio"),
                    os.path.join(local, "ramdisk.cpio.orig"),
                )
            else:
                self.SKIPBACKUP = "#"
    ...
    def patch(self):
        ...
        with open(
            os.path.join(local, "config"), "w", encoding="utf-8", newline="\n"
        ) as config:
        ...
        if os.path.exists(os.path.join(self.Magisk_dir, "magisk32")):
            self.exec(
                "compress=xz", os.path.join(self.Magisk_dir, "magisk32"), "magisk32.xz"
            )
        else:
            self.SKIP32 = "#"
        if os.path.exists(os.path.join(self.Magisk_dir, "magisk64")):
            self.exec(
                "compress=xz", os.path.join(self.Magisk_dir, "magisk64"), "magisk64.xz"
            )
        else:
            self.SKIP64 = "#"
        if os.path.exists(os.path.join(self.Magisk_dir, "stub.apk")):
            self.exec(
                "compress=xz", os.path.join(self.Magisk_dir, "stub.apk"), "stub.xz"
            )
        else:
            self.SKIPSTUB = "#"
        self.exec(
            ...
            if not zipfile.is_zipfile(self.MAGISKAPK):
                ...
                with zipfile.ZipFile(self.MAGISKAPK) as ma:
                    ...
                    arch = [
                        i.split("/")[1].strip()
                        for i in namelist
                        if i.startswith("lib") and i.endswith("libmagiskboot.so")
                    ]
                    ...
                    for patch_arch in patch_archs:
                        for i in [
                            ...
                            for i in namelist
                            if patch_arch in i
                            and os.path.basename(i).startswith("libmagisk")
                        ]:
                            if os.path.basename(i) in [
                                "libmagiskboot.so",
                                "libmagiskpolicy.so",
                            ]:
                                continue
                    ...
                    if "assets/stub.apk" in namelist:
                        ...
                        shutil.copyfile(
                            os.path.join(custom, "assets/stub.apk"),
                            (os.path.join(custom, "stub.apk")),
                        )
                        ...
                        if os.path.isfile(os.path.join(custom, i)):
                            if os.path.basename(i) in lib_library.keys():
                                shutil.move(
                                    os.path.join(custom, i),
                                    os.path.join(
                                        custom, lib_library[os.path.basename(i)]
                                    ),
                                )
